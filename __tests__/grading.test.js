...
describe("Grading Tests:", function () {
   ...
   it("HTML includes external CSS script", function () {
      let linkElements = document.getElementsByTagName("link"); // Use lowercase "link"
      expect(linkElements.length).toBeGreaterThan(0); // Ensure at least one <link> exists
      expect(linkElements[0].href.includes('styles.css')).toBe(true);
   });
